...
namespace LizardSkin
{
    ...
    {
		CosmeticTailFinData tailFinData => cosmeticData as CosmeticTailFinData;
		...
		public override void ApplyPalette(LeaserAdaptor sLeaser, CameraAdaptor rCam, PaletteAdaptor palette)
		{
			for (int i = 0; i < 2; i++)
			{
				...
				{
					float f = Mathf.Lerp(tailFinData.start, tailFinData.length, Mathf.InverseLerp((float)this.startSprite, (float)(this.startSprite + this.bumps - 1), (float)i));
