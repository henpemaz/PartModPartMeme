...
namespace SplitScreenMod
{
    public partial class SplitScreenMod : BaseUnityPlugin
    {
        ...
        bool rrNestedLock;
        private bool RoomRealizer_CanAbstractizeRoom(On.RoomRealizer.orig_CanAbstractizeRoom orig, RoomRealizer self, object tracker)
        {
            ...
            if (!rrNestedLock && realizer2 != null) // if other exists, not recursive
            {
                ...
                {
                    rrNestedLock = true;
                    r = r && other.CanAbstractizeRoom(tracker as RoomRealizer.RealizedRoomTracker);
                    rrNestedLock = false;
                }
            }
            return r;
