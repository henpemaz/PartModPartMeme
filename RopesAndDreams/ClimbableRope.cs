...
namespace RopesAndDreamsMods
{
    public class ClimbableRope : UpdatableAndDeletable, IClimbableVine, IDrawable
    {
        ...
        const float elasticCoef = 0.5f;
        const float dampingFactor = 0.2f;
        const float transmissionFactor = 0.9f;

        const float stiffnessCoef = 0.000f;
        const float stiffnessDampCoef = 0.002f;
        
        const float airFrictionA = 0.002f;
        const float airFrictionB = 0.0001f;

        const float transfDisplace = 0.2f;
        const float transfSpeed = 0.8f;
        const float externalTransfDisplace = 0f;
        const float externalTransfSpeed = 0.8f;


        public ClimbableRope(PlacedObject placedObject, Room room)
        {
        ...
        public override void Update(bool eu)
        {
            ...
            for (int i = 1; i < this.ropes.Length; i++)
            {
                ...
                float deltaTwist = twist - twists[i, 1];

                Vector2 reaction = perp * conRad * (twist * stiffnessCoef + deltaTwist * stiffnessDampCoef);
            ...
            for (int i = this.ropes.Length - 1; i >= 0; i--)
            {
                ...
                Vector2 tangential = perpB * Vector2.Dot(speeds[i + 1, 0], perpB);
                speeds[i, 0] += (speeds[i + 1, 0] - tangential) * transmissionFactor;
            ...
            for (int i = 0; i < this.ropes.Length; i++)
            {
                ...
                this.speeds[i + 1, 0] *= 1 - (airFrictionA + this.speeds[i + 1, 0].magnitude * airFrictionB);
                this.lengths[i, 1] = this.lengths[i, 0];
                this.twists[i, 1] = this.twists[i, 0];

                this.nodes[i + 1, 0] += this.speeds[i + 1, 0];
        ...
        private void PushNode(int index, Vector2 movement)
        {
            if (index > 0 && index < nodeCount)
            {
                this.nodes[index, 2] += movement * transfSpeed;
                this.nodes[index, 0] += movement * transfDisplace;
            }
        }
        ...
        // IDrawable stuff goes all to the sub class
