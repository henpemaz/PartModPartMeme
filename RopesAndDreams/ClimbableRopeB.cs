...
{
    public class ClimbableRopeB : UpdatableAndDeletable, IClimbableVine, IDrawable
    {
        Vector2 startPos;
        Vector2 endPos;
        float length;
        private int nodeCount;
        private float conRad = 10f;
        ...
        const float elasticCoef = 0.8f;
        const float dampingFactor = 0.1f;
        const float transmissionFactor = 0.99f;

        const float stiffnessCoef = 0.000f;
        const float stiffnessDampCoef = 0.005f;

        const float airFrictionA = 0.002f;
        const float airFrictionB = 0.0001f;

        const float externalTransfDisplace = 0.5f;
        const float externalTransfSpeed = 1f;


        public ClimbableRopeB(PlacedObject placedObject, Room room)
        {
